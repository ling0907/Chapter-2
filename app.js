...
function submitScatterData() {
    const dataInput = document.getElementById('scatter-data').value;
    ...
    for (const line of lines) {
        const parts = line.split(',');
        if (parts.length !== 2) {
            showNotification('数据格式错误，请使用 x,y 格式', true);
            return;
        }
    ...
    // 清空输入框
    document.getElementById('scatter-data').value = '';
}
